...
export default function Home() {
  ...
  function handleDateClick(date, allDay) {
    setNewEvent({ ...newEvent, date: date, allDay: allDay, id: new Date().getTime()})
    setShowModals(true)
  }
  ...
  return (
    <>
      <nav className="flex justify-between mb-12 border-b border-violet-100 p-4">
        <h1 className="font-bold to-gray-700 text-2xl">
          <Link href="/">
            Calender-Scheduling
          </Link>
        </h1>
        ...
        <div className="grid grid-cols-10">
          ...
          <div id="draggable-el" className="ml-8 w-full border-2 p-2 rounded-md mt-16 lg:h-1/2 bg-violet-50">
            ...
            {events.map((event, index) => (
              <div
                key={index}
                className="fc-event border-2 m-2 p-1 w-full rounded-md ml-auto text-center bg-white"
              >
                {event.title}
              </div>
            ))}
